...
!
!Input: the index of the element (iele)
!Output: posarr and rhoarr are coordinate(Bohr) and rho at each point, the npt is the actual number of data point for this element
!The posarr and rhoarr passed into this routine must be in the dimension of 200
!
!Beware that the densities are truncated at 1D-8. We can't tighten this criterion, e.g. 1D-100, becuase if the value is very small, Lagrange interpolation gives frantic result!
subroutine genatmraddens(iele,posarr,rhoarr,npt)
implicit real*8 (a-h,o-z)
integer iele,npt
integer,parameter :: nptupper=200 !200 is also the value used in sphatmraddens subroutine, they must be exactly the same
real*8 posarr(nptupper),rhoarr(nptupper)
!Generate radial position array for all elements by using Becke transform
! itmp=0
! do irad=nptupper,1,-1
! 	itmp=itmp+1
! 	radx=cos(irad*3.141592653589793D0/(nptupper+1))
