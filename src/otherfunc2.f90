...
else if (allocated(CObasa).and.wfntype/=2) then !For RO form, HOMO and LUMO are undefined
    write(*,*) "You can also input for example: (quotation marks should not be inputted)"
...
if (index(c1000tmp,'h')==0.and.index(c1000tmp,'l')==0) then !Use inputted a series of orbital indices
    ...
    call getHOMOidx
    tmpval=0
    if (c1000tmp(1:2)=="ha") then
        if (c1000tmp(3:3)=='-') read(c1000tmp(3:),*) tmpval
        iorb=idxHOMO+tmpval
    else if (c1000tmp(1:2)=="hb") then
        if (c1000tmp(3:3)=='-') read(c1000tmp(3:),*) tmpval
        iorb=idxHOMOb+tmpval
    else if (c1000tmp(1:1)=="h") then
        if (c1000tmp(2:2)=='-') read(c1000tmp(2:),*) tmpval
        iorb=idxHOMO+tmpval
    else if (c1000tmp(1:2)=="la") then
        if (c1000tmp(3:3)=='+') read(c1000tmp(3:),*) tmpval
        iorb=idxHOMO+1+tmpval
    else if (c1000tmp(1:2)=="lb") then
        if (c1000tmp(3:3)=='+') read(c1000tmp(3:),*) tmpval
        iorb=idxHOMOb+1+tmpval
    else if (c1000tmp(1:1)=="l") then
        if (c1000tmp(2:2)=='+') read(c1000tmp(2:),*) tmpval
        iorb=idxHOMO+1+tmpval
    end if
    write(*,"(' The index of you selected orbital is',i7,/)") iorb
    if (iorb<1.or.iorb>nmo) then
	    write(*,*) "Error: The orbital you selected exceeded valid range!"
	    return
    end if
    call setgrid(0,itmp)
    if (allocated(cubmat)) deallocate(cubmat)
...
!15=Calculate alpha in a range of frequencies
	if (isel==1.or.isel==5) then
		write(*,*) "Input frequency of external field w for alpha(-w;w), e.g. 0.25"
		write(*,*) "Note: Negative value means using nm as unit, e.g. -693.5"
		write(*,*) "Note: 0 corresponds to static case"
		read(*,*) freq
		if (freq<0) freq=1240.7011D0/au2eV/abs(freq)
		if (freq/=0) then
			wavlen=1240.7011D0/(freq*au2eV)
...
if (selectyn=='y'.or.selectyn=='Y') then
    ...
    do itmp=4,idxend
        ...
        latm=atmseq(ldx)
        dih=xyz2dih(a(iatm)%x,a(iatm)%y,a(iatm)%z,a(jatm)%x,a(jatm)%y,a(jatm)%z,a(katm)%x,a(katm)%y,a(katm)%z,a(latm)%x,a(latm)%y,a(latm)%z)
        !Note that the return value of xyz2dih is [0,180]
        dev=dih
...
real*8,allocatable :: BOM1(:,:),BOM2(:,:),AOM1(:,:),AOM2(:,:)
real*8,allocatable :: BOM1b(:,:),BOM2b(:,:),AOM1b(:,:),AOM2b(:,:)
