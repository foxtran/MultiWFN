...
integer :: itask=1,idispcorr=0,imolden=0,ithermostat=0,ibarostat=0,iSCCS=0,idipcorr=0,iMP2=0,imoment=0,ioptmethod=1,iprintlevel=1
...
do while(.true.)
    ...
    !Task specific options have index >=10
    if (itask==6) then
...
if (index(method,"B3LYP")/=0.or.index(method,"PBE0")/=0.or.index(method,"MP2")/=0) then
        ...
        if (ithermostat>0) then
            ...
            if (nthermoatm<ncenter) then
                ...
                if (ifconti==1) then
                    ...
                    write(ifileid,"(12i6)") thermoatm(1:nthermoatm)
                end if
...
nowcen=0
indcen=0
basstart=0 !X atom without basis function will have 0 for basstart and basend
basend=0
do ibasis=1,nbasis
	if (bascen(ibasis)/=nowcen) then
		nowcen=bascen(ibasis)
