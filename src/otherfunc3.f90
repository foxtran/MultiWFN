...
integer :: numpt=600,icolorarrow=1
...
do while(.true.)
    ...
    if (isel==-7) then
        ...
        write(*,*) "Note: The number will be maginally adjusted by program"
        read(*,*) numpt
    else if (isel==-1) then
        ...
        if (icolorarrow==0) then
            ...
            write(*,"(' Minimal arrow length:',f12.3)") valmin
            write(*,"(' Maximal arrow length:',f12.3)") valmax
            do ipt=1,numpt
                tmp=dsqrt(sum(allvec(:,ipt)**2))
